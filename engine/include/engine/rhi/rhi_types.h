...
enum class TextureFilterMode { Nearest, Linear, NearestMipmapNearest, NearestMipmapLinear, LinearMipmapNearest, LinearMipmapLinear };

enum class TextureType { Texture2D, Texture3D, TextureCube };

enum class ShaderStage { Vertex, Fragment, Geometry, Compute };

enum class VertexAttributeType { Float, Float2, Float3, Float4, Int, Int2, Int3, Int4, UInt, UInt2, UInt3, UInt4 };
...
struct VertexBinding {
    BufferHandle buffer;
    VertexLayout layout;
    uint32_t     divisor = 0;  // 0 for per-vertex, 1 for per-instance
};
...
inline bool IsValid(BufferHandle h) {
    return h.id != 0;
}
inline bool IsValid(TextureHandle h) {
    return h.id != 0;
}
inline bool IsValid(SamplerHandle h) {
    return h.id != 0;
}
inline bool IsValid(ShaderHandle h) {
    return h.id != 0;
}
inline bool IsValid(PipelineHandle h) {
    return h.id != 0;
}
inline bool IsValid(FramebufferHandle h) {
    return h.id != 0;
}
inline bool IsValid(VertexArrayHandle h) {
    return h.id != 0;
}
...
enum class CubemapFace { PositiveX = 0, NegativeX = 1, PositiveY = 2, NegativeY = 3, PositiveZ = 4, NegativeZ = 5 };
