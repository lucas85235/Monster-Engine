...
enum class ShaderStageType { Vertex, Fragment, Geometry, TessControl, TessEvaluation, Compute };
...
class ShaderCrossCompiler {
   public:
     ...
     */
    static CrossCompileResult Process(const std::vector<uint32_t>& spirvBinary, RenderAPI targetAPI, ShaderStageType stage,
        ...
        try {
            if (targetAPI == RenderAPI::Vulkan) {
                ...
                result.success     = true;
                std::cout << "[ShaderCrossCompiler] Vulkan: SPIR-V passthrough (" << spirvBinary.size() * 4 << " bytes)" << std::endl;
                ...
                std::cout << "[ShaderCrossCompiler] OpenGL: Transpiled to GLSL " << glslVersion << " (" << result.glslSource.size() << " chars)"
                          << std::endl;
#else
                result.success      = false;
                result.errorMessage = "SPIRV-Cross not available. Build with -DUSE_SPIRV_CROSS=ON";
   ...
   private:
     ...
     */
    static std::string TranspileToGLSL(const std::vector<uint32_t>& spirvBinary, uint32_t glslVersion, ShaderStageType stage) {
        ...
        // FORCE flattening for now to support legacy Material system which uses glUniform*
        ...
        if (glslSource.find("#version") == std::string::npos) {
            ...
            if (glslVersion >= 140) { versionDirective += " core"; }
            versionDirective += "\n";
   ...
   public:
    ...
    static void RemoveExplicitBindings(spirv_cross::CompilerGLSL& compiler) {
        ...
        for (auto& ubo : resources.uniform_buffers) { compiler.unset_decoration(ubo.id, spv::DecorationBinding); }

        // Unset bindings for samplers/textures
        for (auto& sampler : resources.sampled_images) { compiler.unset_decoration(sampler.id, spv::DecorationBinding); }

        // Unset bindings for separate images
        for (auto& image : resources.separate_images) { compiler.unset_decoration(image.id, spv::DecorationBinding); }

        // Unset bindings for separate samplers
        for (auto& sampler : resources.separate_samplers) { compiler.unset_decoration(sampler.id, spv::DecorationBinding); }

        // Unset bindings for storage buffers (SSBOs)
        for (auto& ssbo : resources.storage_buffers) { compiler.unset_decoration(ssbo.id, spv::DecorationBinding); }
    }
