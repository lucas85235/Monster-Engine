...
CameraController::CameraController(Camera& camera) : camera_(camera) {
    ...
    // For now, we assume bindings "MoveForward", "MoveRight", "MoveUp", "LookX", "LookY" exist or
    // we check raw keys if we want fallback. But the plan was to use InputManager.

    // Let's assume the user of this class ensures bindings are set, OR we check specific keys as
    // fallback? No, let's stick to the plan: use InputManager. But to make it "plug and play" for
    // the existing app, maybe we should bind defaults here if they don't exist? Better: The
    // AppLayer should set up bindings.
}
...
void CameraController::OnUpdate(float ts) {
    ...
    if (moveUp != 0.0f) { camera_.ProcessKeyboard(Camera::CameraMovement::UP, moveUp * velocity); }
    ...
    if (lookX != 0.0f || lookY != 0.0f) { camera_.ProcessMouseMovement(lookX, lookY); }
}
