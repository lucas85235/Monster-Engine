...
void PhysicsSystem::Initialize(const PhysicsConfig& config) {
    ...
    dynamics_world_ =
        new btDiscreteDynamicsWorldMt(dispatcher_, overlapping_pair_cache_broadphase_interface_, solver_pool_, solver_, collision_configuration_);
...
void PhysicsSystem::Update(float dt) {
    ...
    // Update profiler metrics
    {
        ...
        PerformanceProfiler::Get().SetPhysicsMetrics(active_body_count, sleeping, bodies_.size(), all_bodies_sleeping_, last_physics_execution_time_);
        PerformanceProfiler::Get().SetThreadPoolInfo(0, task_scheduler_ ? task_scheduler_->getNumThreads() : 0);
    }
    ...
    if (active_body_count > 0) {
        for (auto& entry : bodies_) {
            ...
            transform_component.Position =
                glm::vec3(static_cast<float>(origin.getX()), static_cast<float>(origin.getY()), static_cast<float>(origin.getZ()));
...
btRigidBody* PhysicsSystem::AddRigidBody(Entity entity, const RigidbodyData& data) {
    ...
    bool uniform_scale = std::abs(scale.x() - 1.0f) < 0.01f && std::abs(scale.y() - 1.0f) < 0.01f && std::abs(scale.z() - 1.0f) < 0.01f;
    ...
    if (is_trigger) { rigid_body->setCollisionFlags(rigid_body->getCollisionFlags() | btCollisionObject::CF_NO_CONTACT_RESPONSE); }

    btVector3 angular_factor(data.freezeRotationX ? 0.0f : 1.0f, data.freezeRotationY ? 0.0f : 1.0f, data.freezeRotationZ ? 0.0f : 1.0f);
