...
Rml::CompiledGeometryHandle RmlUiRenderInterface::CompileGeometry(
    ...
    glBufferData(GL_ARRAY_BUFFER, vertices.size() * sizeof(Rml::Vertex), vertices.data(),
                 GL_STATIC_DRAW);
...
void RmlUiRenderInterface::RenderGeometry(Rml::CompiledGeometryHandle geometry,
    ...
    // Set Projection
    glm::mat4 projection =
        glm::ortho(0.0f, (float)viewport_width_, (float)viewport_height_, 0.0f, -1.0f, 1.0f);
...
void RmlUiRenderInterface::SetScissorRegion(Rml::Rectanglei region) {
    ...
    glScissor(region.Left(), viewport_height_ - (region.Top() + region.Height()), region.Width(),
              region.Height());
}
...
Rml::TextureHandle RmlUiRenderInterface::LoadTexture(Rml::Vector2i&     texture_dimensions,
    ...
    Rml::TextureHandle handle =
        GenerateTexture(Rml::Span<const Rml::byte>(data, width * height * 4), texture_dimensions);
...
bool RmlUiFontEngineInterface::LoadFontFace(const Rml::String& file_name, int face_index,
                                            bool fallback_face, Rml::Style::FontWeight weight) {
    return true;
}

bool RmlUiFontEngineInterface::LoadFontFace(Rml::Span<const Rml::byte> data, int face_index,
                                            const Rml::String& family, Rml::Style::FontStyle style,
                                            Rml::Style::FontWeight weight, bool fallback_face) {
    return true;
}

Rml::FontFaceHandle RmlUiFontEngineInterface::GetFontFaceHandle(const Rml::String&     family,
...
Rml::FontEffectsHandle RmlUiFontEngineInterface::PrepareFontEffects(
...
int RmlUiFontEngineInterface::GetStringWidth(Rml::FontFaceHandle handle, Rml::StringView string,
                                             const Rml::TextShapingContext& text_shaping_context,
                                             Rml::Character                 prior_character) {
    return 100;  // Dummy fixed width
}

int RmlUiFontEngineInterface::GenerateString(Rml::RenderManager&    render_manager,
