...
bool VulkanDevice::Initialize() {
    ...
    cachedUbo.view[0] = cachedUbo.view[5] = cachedUbo.view[10] = cachedUbo.view[15] = 1.0f;
    cachedUbo.proj[0] = cachedUbo.proj[5] = cachedUbo.proj[10] = cachedUbo.proj[15] = 1.0f;

    // Initialize push constants (per-draw data)
    memset(cachedPushConstants.model, 0, sizeof(cachedPushConstants.model));
    cachedPushConstants.model[0] = cachedPushConstants.model[5] = cachedPushConstants.model[10] = cachedPushConstants.model[15] = 1.0f;
...
void VulkanDevice::Shutdown() {
    ...
    for (auto& pair : samplers) { vkDestroySampler(device, pair.second.sampler, nullptr); }
...
DeviceInfo VulkanDevice::GetDeviceInfo() const {
    ...
    if (physicalDevice == VK_NULL_HANDLE) return info;
    ...
    info.apiVersion   = std::to_string(VK_VERSION_MAJOR(props.apiVersion)) + "." + std::to_string(VK_VERSION_MINOR(props.apiVersion)) + "." +
...
void VulkanDevice::pickPhysicalDevice() {
    ...
    if (deviceCount == 0) { throw std::runtime_error("[Vulkan] Failed to find GPUs with Vulkan support"); }
    ...
    if (physicalDevice == VK_NULL_HANDLE) { throw std::runtime_error("[Vulkan] Failed to find a suitable GPU"); }
...
VkPresentModeKHR VulkanDevice::chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& presentModes) {
...
void VulkanDevice::createDepthResources() {
    ...
    if (vkCreateImage(device, &imageInfo, nullptr, &depthImage) != VK_SUCCESS) { throw std::runtime_error("[Vulkan] Failed to create depth image"); }
...
void VulkanDevice::createDummyTexture() {
    ...
    createImage(1, 1, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
                VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, dummyImage, dummyImageMemory);

    transitionImageLayout(dummyImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
    copyBufferToImage(stagingBuffer, dummyImage, 1, 1);
    transitionImageLayout(dummyImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
...
void VulkanDevice::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage,
    ...
    if (vkCreateImage(device, &imageInfo, nullptr, &image) != VK_SUCCESS) { throw std::runtime_error("[Vulkan] Failed to create image"); }
...
void VulkanDevice::UpdateBuffer(BufferHandle buffer, const void* data, uint32_t size, uint32_t offset) {
...
TextureHandle VulkanDevice::CreateTexture(const TextureDescriptor& desc) {
    ...
    vkCmdPipelineBarrier(commandBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, nullptr, 0, nullptr, 1,
                         &barrier);
    ...
    if (desc.data && !vkTexture.isCubemap) {
        VkDeviceSize imageSize = static_cast<VkDeviceSize>(desc.width) * static_cast<VkDeviceSize>(desc.height) * 4;
        ...
        bufAllocInfo.memoryTypeIndex =
            findMemoryType(bufMemReq.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
        ...
        vkCmdCopyBufferToImage(cmd, stagingBuffer, vkTexture.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
    ...
    viewInfo.viewType                        = vkTexture.isCubemap ? VK_IMAGE_VIEW_TYPE_CUBE : VK_IMAGE_VIEW_TYPE_2D;
...
void VulkanDevice::UpdateTexture(TextureHandle texture, const void* data, uint32_t mipLevel) {
    ...
    transitionImageLayout(it->second.image, it->second.format, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
    copyBufferToImage(stagingBuffer, it->second.image, it->second.width, it->second.height);
    transitionImageLayout(it->second.image, it->second.format, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

    vkDestroyBuffer(device, stagingBuffer, nullptr);
    vkFreeMemory(device, stagingBufferMemory, nullptr);
}

void VulkanDevice::UpdateTextureCubeFace(TextureHandle texture, CubemapFace face, const void* data, uint32_t mipLevel) {
    auto it = textures.find(texture.id);
    if (it == textures.end()) return;
    ...
    vkCmdPipelineBarrier(commandBuffer, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr, 1,
                         &barrier);
    ...
    vkCmdCopyBufferToImage(commandBuffer, stagingBuffer, it->second.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
...
void VulkanDevice::GenerateMipmaps(TextureHandle texture) {
    ...
    for (uint32_t i = 1; i < mipLevels; i++) {
        ...
        vkCmdBlitImage(commandBuffer, tex.image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, tex.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &blit,
                       VK_FILTER_LINEAR);
...
ShaderHandle VulkanDevice::CreateShader(const std::vector<ShaderDescriptor>& stages) {
...
FramebufferHandle VulkanDevice::CreateFramebuffer(const FramebufferDescriptor& desc) {
    ...
    subpass.pDepthStencilAttachment = (desc.hasDepth && IsValid(desc.depthAttachment.texture)) ? &depthRef : nullptr;
...
void VulkanDevice::AttachTexture(FramebufferHandle framebuffer, FramebufferAttachment attachment, TextureHandle texture) {
    // TODO: Implement
}

TextureHandle VulkanDevice::GetFramebufferTexture(FramebufferHandle framebuffer, FramebufferAttachment attachment) {
    auto it = framebuffers.find(framebuffer.id);
    if (it == framebuffers.end()) return TextureHandle{0};

    if (attachment == FramebufferAttachment::Depth || attachment == FramebufferAttachment::DepthStencil) { return it->second.depthAttachment; }

    int colorIndex = static_cast<int>(attachment) - static_cast<int>(FramebufferAttachment::Color0);
    if (colorIndex >= 0 && colorIndex < static_cast<int>(it->second.colorAttachments.size())) { return it->second.colorAttachments[colorIndex]; }
    return TextureHandle{0};
}

void VulkanDevice::ResizeFramebuffer(FramebufferHandle framebuffer, uint32_t width, uint32_t height) {
    auto it = framebuffers.find(framebuffer.id);
    ...
    for (const auto& texHandle : colorAttachments) {
        ...
        if (texIt != textures.end()) { attachmentViews.push_back(texIt->second.imageView); }
    }
...
void VulkanDevice::DestroyFramebuffer(FramebufferHandle framebuffer) {
    ...
    if (it->second.framebuffer != VK_NULL_HANDLE) { vkDestroyFramebuffer(device, it->second.framebuffer, nullptr); }
    if (it->second.ownsRenderPass && it->second.renderPass != VK_NULL_HANDLE) { vkDestroyRenderPass(device, it->second.renderPass, nullptr); }
...
void VulkanDevice::SetUniform(ShaderHandle shader, const std::string& name, int value) {
    bool updated = false;
    ...
    if (name.find("hasTextureDiffuse") != std::string::npos) {
        ...
        // Sync UBO to GPU
        if (currentFrame < uniformBuffersMapped.size() && uniformBuffersMapped[currentFrame]) {
            memcpy(uniformBuffersMapped[currentFrame], &cachedUbo, sizeof(UniformBufferObject));
        }
    }

    if (updated) { cachedPushConstants.materialProps[3] = static_cast<float>(currentFlags); }
}

void VulkanDevice::SetUniform(ShaderHandle shader, const std::string& name, float value) {
    bool updated = false;
    ...
    if (name.find("roughness") != std::string::npos) {
        ...
        updated                              = true;
    } else if (name.find("ao") != std::string::npos && name.find("emission") == std::string::npos) {
    ...
    if (updated && currentFrame < uniformBuffersMapped.size() && uniformBuffersMapped[currentFrame]) {
        memcpy(uniformBuffersMapped[currentFrame], &cachedUbo, sizeof(UniformBufferObject));
    }
}

void VulkanDevice::SetUniform(ShaderHandle shader, const std::string& name, const float* value, uint32_t count) {
    bool updated = false;
    ...
    if (name.find("albedo") != std::string::npos && count >= 3) {
        ...
        updated              = true;
    } else if ((name.find("lightDir") != std::string::npos || name.find("dirLight.direction") != std::string::npos) && count >= 3) {
        cachedUbo.lightDir[0] = value[0];
        cachedUbo.lightDir[1] = value[1];
        cachedUbo.lightDir[2] = value[2];
        updated               = true;
    } else if ((name.find("lightColor") != std::string::npos || name.find("dirLight.color") != std::string::npos) && count >= 3) {
...
void VulkanDevice::EndFrame() {
    ...
    if (vkEndCommandBuffer(commandBuffers[currentFrame]) != VK_SUCCESS) { throw std::runtime_error("[Vulkan] Failed to record command buffer"); }
...
void VulkanDevice::DrawSkybox(TextureHandle cubemap, SamplerHandle sampler, const float* viewMatrix, const float* projMatrix) {
    auto texIt  = textures.find(cubemap.id);
    auto sampIt = samplers.find(sampler.id);
    if (texIt == textures.end() || sampIt == samplers.end()) { return; }
    ...
    vkCmdBindPipeline(commandBuffers[currentFrame], VK_PIPELINE_BIND_POINT_GRAPHICS, skyboxPipeline);

    // Bind skybox descriptor set
    vkCmdBindDescriptorSets(commandBuffers[currentFrame], VK_PIPELINE_BIND_POINT_GRAPHICS, skyboxPipelineLayout, 0, 1,
...
void VulkanDevice::initializeSkyboxResources() {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        vkMapMemory(device, skyboxUBOMemories[i], 0, uboSize, 0, &skyboxUBOMapped[i]);
    }
    ...
    vkCreatePipelineLayout(device, &pipeLayoutInfo, nullptr, &skyboxPipelineLayout);
...
void VulkanDevice::cleanupSkyboxResources() {
    ...
    if (skyboxDescriptorPool != VK_NULL_HANDLE) vkDestroyDescriptorPool(device, skyboxDescriptorPool, nullptr);
    if (skyboxDescriptorSetLayout != VK_NULL_HANDLE) vkDestroyDescriptorSetLayout(device, skyboxDescriptorSetLayout, nullptr);
    if (skyboxCubeVB != VK_NULL_HANDLE) vkDestroyBuffer(device, skyboxCubeVB, nullptr);
    if (skyboxCubeVBMemory != VK_NULL_HANDLE) vkFreeMemory(device, skyboxCubeVBMemory, nullptr);

    for (size_t i = 0; i < skyboxUBOs.size(); i++) {
        if (skyboxUBOs[i] != VK_NULL_HANDLE) vkDestroyBuffer(device, skyboxUBOs[i], nullptr);
        if (skyboxUBOMemories[i] != VK_NULL_HANDLE) vkFreeMemory(device, skyboxUBOMemories[i], nullptr);
    }
