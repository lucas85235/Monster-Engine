...
bool OpenGLDevice::Initialize() {
    ...
    if (GLAD_GL_VERSION_4_5) {
        ...
        std::cout << "[OpenGL] Depth range set to [0,1] (Vulkan-compatible)" << std::endl;
    } else {
        std::cout << "[OpenGL] Warning: OpenGL 4.5 not supported, depth range might be incorrect" << std::endl;
    }
...
void OpenGLDevice::Shutdown() {
    for (auto& [id, buf] : buffers) glDeleteBuffers(1, &buf.id);
    for (auto& [id, tex] : textures) glDeleteTextures(1, &tex.id);
    for (auto& [id, smp] : samplers) glDeleteSamplers(1, &smp.id);
    for (auto& [id, shd] : shaders) glDeleteProgram(shd.program);
    for (auto& [id, vao] : vertexArrays) glDeleteVertexArrays(1, &vao.vao);
...
GLenum OpenGLDevice::ToGLFramebufferAttachment(FramebufferAttachment attachment) {
    switch (attachment) {
...
void OpenGLDevice::ApplyBlendState(const BlendState& state) {
    if (state.blendEnable) {
        glEnable(GL_BLEND);
        glBlendFuncSeparate(ToGLBlendFactor(state.srcColorFactor), ToGLBlendFactor(state.dstColorFactor), ToGLBlendFactor(state.srcAlphaFactor),
                            ToGLBlendFactor(state.dstAlphaFactor));
        glBlendEquationSeparate(ToGLBlendOp(state.colorOp), ToGLBlendOp(state.alphaOp));
    } else {
        glDisable(GL_BLEND);
    }
...
void OpenGLDevice::UpdateBuffer(BufferHandle buffer, const void* data, uint32_t size, uint32_t offset) {
...
TextureHandle OpenGLDevice::CreateTexture(const TextureDescriptor& desc) {
    ...
    if (desc.type == TextureType::Texture2D) {
        ...
        glTexImage3D(GL_TEXTURE_3D, 0, internalFormat, desc.width, desc.height, desc.depth, 0, format, type, desc.data);
    } else if (desc.type == TextureType::TextureCube) {
        for (int i = 0; i < 6; i++) {
            glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, internalFormat, desc.width, desc.height, 0, format, type, nullptr);
        }
    }

    if (desc.generateMipmaps) { glGenerateMipmap(texture.target); }
...
void OpenGLDevice::UpdateTexture(TextureHandle texture, const void* data, uint32_t mipLevel) {
    ...
    glTexSubImage2D(it->second.target, mipLevel, 0, 0, it->second.width, it->second.height, format, type, data);

    glBindTexture(it->second.target, 0);
}

void OpenGLDevice::UpdateTextureCubeFace(TextureHandle texture, CubemapFace face, const void* data, uint32_t mipLevel) {
    auto it = textures.find(texture.id);
    if (it == textures.end()) return;

    if (it->second.target != GL_TEXTURE_CUBE_MAP) {
        std::cerr << "[OpenGL] UpdateTextureCubeFace: texture is not a cubemap" << std::endl;
        return;
    }
    ...
    glTexImage2D(faceTarget, mipLevel, internalFormat, it->second.width, it->second.height, 0, format, type, data);
...
GLuint OpenGLDevice::CompileShaderStage(const std::string& source, GLenum stage) {
...
GLuint OpenGLDevice::LinkProgram(const std::vector<GLuint>& stages) {
    ...
    for (GLuint stage : stages) { glDeleteShader(stage); }

    return program;
}

ShaderHandle OpenGLDevice::CreateShader(const std::vector<ShaderDescriptor>& stages) {
    ...
    for (const auto& stage : stages) {
        ...
        if (stage.useSPIRV && !stage.spirvBinary.empty()) {
            ...
            auto result = ShaderCrossCompiler::Process(stage.spirvBinary, RenderAPI::OpenGL, stageType, 450);

            if (!result.success) {
                std::cerr << "[OpenGL] SPIR-V transpilation failed: " << result.errorMessage << std::endl;
...
#else
            std::cerr << "[OpenGL] SPIRV-Cross not available, cannot use SPIR-V shaders" << std::endl;
            return ShaderHandle{0};
#endif
        }

        GLuint compiled = CompileShaderStage(glslSource, ToGLShaderStage(stage.stage));
        if (compiled == 0) {
            for (GLuint s : compiledStages) glDeleteShader(s);
            return ShaderHandle{0};
        }
    ...
    if (program == 0) { return ShaderHandle{0}; }
...
PipelineHandle OpenGLDevice::CreatePipeline(const PipelineDescriptor& desc, ShaderHandle shader, const VertexLayout& layout) {
...
FramebufferHandle OpenGLDevice::CreateFramebuffer(const FramebufferDescriptor& desc) {
    ...
    for (size_t i = 0; i < desc.colorAttachments.size(); i++) {
        ...
        if (texIt->second.target == GL_TEXTURE_CUBE_MAP) {
            // Attach cubemap face
            GLenum faceTarget = GL_TEXTURE_CUBE_MAP_POSITIVE_X + static_cast<int>(attachment.cubeFace);
            glFramebufferTexture2D(GL_FRAMEBUFFER, glAttachment, faceTarget, texIt->second.id, attachment.mipLevel);
        } else {
            glFramebufferTexture2D(GL_FRAMEBUFFER, glAttachment, texIt->second.target, texIt->second.id, attachment.mipLevel);
        }

        FramebufferAttachment rhiAttachment = static_cast<FramebufferAttachment>(static_cast<int>(FramebufferAttachment::Color0) + i);
    ...
    if (desc.hasDepth) {
        if (IsValid(desc.depthAttachment.texture)) {
            ...
            if (texIt != textures.end()) {
                glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, texIt->second.target, texIt->second.id, desc.depthAttachment.mipLevel);
                fb.attachments[FramebufferAttachment::Depth] = desc.depthAttachment.texture;
            }
        } else {
    ...
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) { std::cerr << "[OpenGL] Framebuffer is not complete!" << std::endl; }
...
void OpenGLDevice::AttachTexture(FramebufferHandle framebuffer, FramebufferAttachment attachment, TextureHandle texture) {
    auto fbIt  = framebuffers.find(framebuffer.id);
    auto texIt = textures.find(texture.id);

    if (fbIt == framebuffers.end() || texIt == textures.end()) return;

    glBindFramebuffer(GL_FRAMEBUFFER, fbIt->second.fbo);
    glFramebufferTexture2D(GL_FRAMEBUFFER, ToGLFramebufferAttachment(attachment), GL_TEXTURE_2D, texIt->second.id, 0);
    glBindFramebuffer(GL_FRAMEBUFFER, 0);

    fbIt->second.attachments[attachment] = texture;
}

TextureHandle OpenGLDevice::GetFramebufferTexture(FramebufferHandle framebuffer, FramebufferAttachment attachment) {
    ...
    if (texIt == it->second.attachments.end()) return TextureHandle{0};

    return texIt->second;
}

void OpenGLDevice::ResizeFramebuffer(FramebufferHandle framebuffer, uint32_t width, uint32_t height) {
    auto it = framebuffers.find(framebuffer.id);
    ...
    for (auto& [attachment, texHandle] : it->second.attachments) {
        ...
        if (texIt != textures.end()) {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, ToGLTextureInternalFormat(texIt->second.format), width, height, 0, ToGLTextureFormat(texIt->second.format),
...
void OpenGLDevice::DestroyFramebuffer(FramebufferHandle framebuffer) {
    ...
    if (it != framebuffers.end()) {
        ...
        if (it->second.rbo) { glDeleteRenderbuffers(1, &it->second.rbo); }

        for (auto& [attachment, texHandle] : it->second.attachments) { DestroyTexture(texHandle); }
...
void OpenGLDevice::DisableScissor() {
...
void OpenGLDevice::BindPipeline(PipelineHandle pipeline) {
    ...
    if (shaderIt != shaders.end()) { glUseProgram(shaderIt->second.program); }
...
void OpenGLDevice::BindVertexArray(VertexArrayHandle vao) {
    auto it = vertexArrays.find(vao.id);
    if (it != vertexArrays.end()) { glBindVertexArray(it->second.vao); }
}
...
void OpenGLDevice::BindSampler(uint32_t slot, SamplerHandle sampler) {
    auto it = samplers.find(sampler.id);
    if (it != samplers.end()) { glBindSampler(slot, it->second.id); }
}

void OpenGLDevice::SetUniform(ShaderHandle shader, const std::string& name, int value) {
    auto it = shaders.find(shader.id);
...
void OpenGLDevice::SetUniform(ShaderHandle shader, const std::string& name, float value) {
    auto it = shaders.find(shader.id);
    if (it == shaders.end()) return;
...
void OpenGLDevice::SetUniform(ShaderHandle shader, const std::string& name, const float* value, uint32_t count) {
    auto it = shaders.find(shader.id);
    if (it == shaders.end()) return;
...
void OpenGLDevice::SetUniformMatrix4(ShaderHandle shader, const std::string& name, const float* matrix) {
    auto it = shaders.find(shader.id);
    if (it == shaders.end()) return;
...
void OpenGLDevice::Draw(const DrawCommand& cmd) {
    ...
    if (cmd.instanceCount > 1) {
        glDrawArraysInstanced(topology, cmd.firstVertex, cmd.vertexCount, cmd.instanceCount);
    } else {
...
void OpenGLDevice::DrawIndexed(const DrawIndexedCommand& cmd) {
    ...
    size_t indexSize = (cmd.indexType == IndexType::UInt16) ? sizeof(uint16_t) : sizeof(uint32_t);

    void* indexOffset = (void*)(uintptr_t)(cmd.firstIndex * indexSize);

    if (cmd.instanceCount > 1) {
        glDrawElementsInstanced(topology, cmd.indexCount, indexType, indexOffset, cmd.instanceCount);
    } else {
...
void OpenGLDevice::initializeSkyboxResources() {
    std::cout << "[OpenGL] Initializing dedicated skybox resources..." << std::endl;
...
void main() {
    ...
    if (!success) {
        ...
        std::cerr << "[OpenGL] Skybox vertex shader error: " << infoLog << std::endl;
    }
    ...
    if (!success) {
        ...
        std::cerr << "[OpenGL] Skybox fragment shader error: " << infoLog << std::endl;
    }
    ...
    std::cout << "[OpenGL] Skybox resources initialized successfully" << std::endl;
}

void OpenGLDevice::DrawSkybox(TextureHandle cubemap, SamplerHandle sampler, const float* viewMatrix, const float* projMatrix) {
    ...
    if (!skyboxInitialized) {
        ...
        if (!skyboxInitialized) {
            std::cerr << "[OpenGL] Failed to initialize skybox resources" << std::endl;
            return;
        }
    }
    ...
    if (sampIt != samplers.end()) { glBindSampler(0, sampIt->second.id); }
    ...
    // Unbind sampler
    if (sampIt != samplers.end()) { glBindSampler(0, 0); }
}
