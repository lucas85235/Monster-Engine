...
void main() {
    ...
    // Use vertex color if not black, otherwise use bright red for visibility
    vec3 baseColor = length(v_Color) > 0.01 ? v_Color : vec3(1.0, 0.3, 0.1);
    
    // Apply material color if set
    baseColor = materialColor.rgb * (1.0 - step(0.001, length(materialColor.rgb))) + baseColor * step(0.001, length(materialColor.rgb));
