#!/usr/bin/env python3
"""
UI Editor — Cross-platform run script.

Usage:
    python tools/ui_editor/scripts/run.py
"""
...
def main() -> None:
    ...
    u.info("========================================")
    u.info(f"  UI Editor - Run Script ({u.detect_platform()})")
    u.info("========================================")
    print()
    ...
    if not exe.exists():
        u.fatal(
            ...
            "       Please run: python tools/ui_editor/scripts/build.py"
        )

    print(f"Starting UI Editor from: {project_root}")
    print()
