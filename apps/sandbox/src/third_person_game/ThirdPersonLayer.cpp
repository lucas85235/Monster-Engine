...
void ThirdPersonLayer::CreateScene() {
    ...
    // Create player cube
    {
        ...
        player_entity_.AddComponent<MeshRenderComponent>(mesh, material_);
        ...
        auto& springArm           = player_entity_.AddComponent<SpringArmComponent>();
    ...
    {
        sun_entity_     = scene_->CreateEntity("Sun");
        auto& transform = sun_entity_.GetComponent<TransformComponent>();
        transform.SetPosition({10.0f, 20.0f, 10.0f});
        transform.SetRotation({45.0f, 45.0f, 0.0f});

        auto& dirLight       = sun_entity_.AddComponent<DirectionalLightComponent>();
...
void ThirdPersonLayer::UpdatePlayer(float ts) {
    auto& input     = InputManager::Get();
    auto& transform = player_entity_.GetComponent<TransformComponent>();
    auto& springArm = player_entity_.GetComponent<SpringArmComponent>();
...
void ThirdPersonLayer::UpdateCamera() {
    ...
    if (!player_entity_.HasComponent<SpringArmComponent>()) return;
    auto& springArm   = player_entity_.GetComponent<SpringArmComponent>();
    auto& playerTrans = player_entity_.GetComponent<TransformComponent>();
...
void ThirdPersonLayer::OnImGuiRender() {
    ...
    if (player_entity_.HasComponent<TransformComponent>()) {
        auto& trans = player_entity_.GetComponent<TransformComponent>();
    ...
    if (player_entity_.HasComponent<SpringArmComponent>()) {
        auto& springArm = player_entity_.GetComponent<SpringArmComponent>();
    ...
    auto& sun_trans = sun_entity_.GetComponent<TransformComponent>();
    ImGui::DragFloat3("Sun Rotation", &sun_trans.Rotation.x, 0.1f);
