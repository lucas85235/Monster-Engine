...
void ThirdPersonLayer::CreateScene() {
    ...
    // Attempt to load backpack model
    LoadModel("assets/models/car/Intergalactic_Spaceship-(Wavefront).obj");
}

void ThirdPersonLayer::LoadModel(const std::string& path) {
    if (!std::filesystem::exists(path)) {
        SE_LOG_WARN("Model file not found: {}", path);
        return;
    }

    SE_LOG_INFO("Loading model: {}", path);
    auto model = ModelLoader::Load(path);
    if (!model) {
        SE_LOG_ERROR("Failed to load model: {}", path);
        return;
    }

    // Create entity for the model root
    auto  modelEntity = scene_->CreateEntity("LoadedModel");
    auto& transform   = modelEntity.GetComponent<TransformComponent>();
    transform.SetPosition({5.0f, 2.0f, 5.0f});
    transform.SetScale({1.0f, 1.0f, 1.0f});

    // Create sub-entities for each mesh
    int i = 0;
    for (const auto& submesh : model->GetSubMeshes()) {
        auto subEntity = scene_->CreateEntity("SubMesh_" + std::to_string(i++));

        // Convert Mesh to VertexArray
        if (!submesh.mesh) continue;
        auto vertexArray = MeshManager::CreateVertexArrayFromMesh(*submesh.mesh);

        auto& meshRender          = subEntity.AddComponent<MeshRenderComponent>(vertexArray, submesh.material);
        meshRender.BoundingRadius = submesh.mesh->CalculateBoundingRadius();

        // Parent to model root (Parenting not yet implemented in Scene, so just copy transform for now or set independent)
        // Assuming simple hierarchy: set same transform
        auto& subTrans = subEntity.GetComponent<TransformComponent>();
        subTrans.SetPosition({5.0f, 2.0f, 5.0f});
        subTrans.SetScale({1.0f, 1.0f, 1.0f});

        // Add collider (Box) for interaction?
        subEntity.AddComponent<BoxCollider>(glm::vec3(1.0f));
        RigidbodyData data;
        data.mass = 1.0f;  // Dynamic!
        subEntity.AddComponent<RigidbodyComponent>(data, subEntity);
    }
}
...
void ThirdPersonLayer::CleanupBullets() {
    ...
    bullets_.erase(std::remove_if(bullets_.begin(), bullets_.end(), [](const Entity& e) { return !e.IsValid(); }), bullets_.end());
}
...
void ThirdPersonLayer::UpdateCamera() {
    ...
    if (springArm.DoCollisionTest && scene_->GetPhysicsSystem()) {
        ...
        if (playerEntity_.HasComponent<RigidbodyComponent>()) { playerBody = playerEntity_.GetComponent<RigidbodyComponent>().GetRigidbody(); }
...
void ThirdPersonLayer::OnImGuiRender() {
    ...
    if (ImGui::Checkbox("Enable Frustum Culling", &enableFrustumCulling_)) { sceneRenderer.SetFrustumCullingEnabled(enableFrustumCulling_); }
    if (ImGui::Checkbox("Enable Occlusion Culling", &enableOcclusionCulling_)) { sceneRenderer.SetOcclusionCullingEnabled(enableOcclusionCulling_); }
