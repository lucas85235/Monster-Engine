...
if [[ "$(uname)" == "Darwin" ]]; then
    ...
    CMAKE_EXTRA_FLAGS="-DCMAKE_BUILD_TYPE=${BUILD_TYPE}"
else
    # Linux — usa clang-17 com libc++ (instalado via setup.sh)
    echo "Detected Linux — using clang-17 with libc++"
    CMAKE_EXTRA_FLAGS="-DCMAKE_BUILD_TYPE=${BUILD_TYPE} -DCMAKE_C_COMPILER=clang-17 -DCMAKE_CXX_COMPILER=clang++-17 -DCMAKE_CXX_FLAGS=-stdlib=libc++"
fi
